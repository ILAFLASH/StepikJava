...
public class Test3 {
    private String printTextPerRole(String[] roles, String[] textLines) {
        StringBuilder[] resultText = new StringBuilder[roles.length];
        String resultString = "";
        // Заполняем массив ролями
        for (int i = 0; i < roles.length; i++) {
            resultText[i] = new StringBuilder();
            resultText[i].append(roles[i] + ":\n");
        }
        for (int i = 0; i < roles.length; i++) {
            for (int j = 0; j < textLines.length; j++) {
                if (textLines[j].matches(roles[i] + ":.*")) {
                    resultText[i].append((j + 1) + ") " + textLines[j].substring(roles[i].length() + 2) + "\n");
                }
            }
        }
        for (StringBuilder s : resultText) {
            resultString += s + "\n";
        }
    ...
    public static void main(String[] args) {
        Test3 t3 = new Test3();
        t3.printTextPerRole(new String[]{"Городничий", "Аммос Федорович", "Артемий Филиппович", "Лука Лукич"},
                new String[]{"Городничий: Я пригласил вас, господа, с тем, чтобы сообщить вам пренеприятное известие: к нам едет ревизор.",
                        ...
                        "Артемий Филиппович: Вот не было заботы, так подай!", "Лука Лукич: Господи боже! еще и с секретным предписаньем!"});
